...
# ---------------------------------------------------------
# Robust Multi-variable Serial Reader QThread
# ---------------------------------------------------------
class SerialReader(QThread):
    # telemetry_received carries: angle (deg), position (m), force (N)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Inverted Pendulum Telemetry Dashboard")
        ...
        # State controllers: Defaults to Hardware Mode on startup
        ...
        # Real-time tick timer (8ms interval matches physics step dt)
        self.timer = QTimer()
    ...
    def init_ui(self):
        ...
        """)
        lbl_subtitle = QLabel("Real-time Hardware Telemetry Dashboard")
        ...
        self.btn_start = QPushButton("Start Link")
    ...
    # ---------------------------------------------------------
    # Telemetry and Connection Controls
    # ---------------------------------------------------------
    ...
    def start_serial(self):
        ...
        if not port:
            self.set_status_indicator("No Port", "red")
            return
    ...
    def set_status_indicator(self, text, state):
        ...
        if text.startswith("Connected"):
            ...
            self.btn_start.setText("Start Link" if not self.is_simulation else "Start")
            self.is_running = False
