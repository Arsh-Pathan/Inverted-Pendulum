...
class TelemetryCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        bg_color = QColor(30, 30, 30) if self.is_dark_mode else QColor(255, 255, 255)
        painter.fillRect(self.rect(), bg_color)

        # CAD Grid overlay
        grid_color = QColor(60, 60, 60) if self.is_dark_mode else QColor(240, 240, 240)
        ...
        cart_color = QColor(80, 80, 80) if self.is_dark_mode else QColor(245, 245, 245)
        painter.setBrush(QBrush(cart_color))
        ...
        rod_color = QColor(255, 255, 255) if self.is_dark_mode else QColor(0, 0, 0)
        painter.setPen(QPen(rod_color, 4, Qt.PenStyle.SolidLine, Qt.PenCapStyle.SquareCap))
        painter.drawLine(QPointF(cx, cy), QPointF(px, py))

        # Bob mass (Black)
        bob_r = 15.0
        painter.setBrush(QBrush(rod_color))
        painter.drawEllipse(QPointF(px, py), bob_r, bob_r)

        # Inner center pin
        painter.setBrush(QBrush(QColor(255, 255, 255)))
        painter.setPen(QPen(rod_color, 1.5))
