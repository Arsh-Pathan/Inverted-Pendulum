...
def train_ppo(timesteps: int = 50000, lr: float = 3e-4, hil_port: str = None,
              save_path: str = "rl/models/ppo_pendulum.zip",
              data_path: str = "rl/training_data/ppo_transitions.csv"):
    ...
    model = PPO(
        ...
        batch_size=64,
        n_epochs=10,
        # At dt = 10 ms, gamma=0.99 discounts to ~1 s of lookahead, far shorter than a
        # balancing episode. gamma=0.999 gives a ~10 s effective horizon, matching the task.
        gamma=0.999,
        gae_lambda=0.95,
        clip_range=0.2,
    ...
    for _ in range(1000):
        ...
        if terminated or truncated:
            break
    print(f"Verification Episode Total Reward: {ep_reward:.2f} (Survived {steps_survived} steps)")
    env.close()
    eval_env.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default="rl/training_data/ppo_transitions.csv", help="CSV path for per-step training data")
    args = parser.parse_args()
    train_ppo(args.timesteps, args.lr, args.hil_port, args.save_path, args.data_path)
